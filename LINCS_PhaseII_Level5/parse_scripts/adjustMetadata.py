...
try:
    ...
    for line in metaIn :
        ...
        lineList = line.strip('\n').split('\t')
        if (lineList[0] != sigId) :
            sigMetricsList = sigMetricsDict[lineList[0]] ##This will add the sigMetrics metadata to the metadata.tsv.gz 
            for i in range(len(sigMetricsList)) :
                if(str(sigMetricsList[i]) != "-666") :
                    metaOut.write(str(lineList[0]) + '\t' + str(sigMetricsHeaderList[i]) + '\t' + str(sigMetricsList[i]) + '\n')
            sigId = lineList[0]
